...
class Tank_auth
{
    ...
    {
        return $this->ci->session->userdata('class_id');
    }
    /**
     * Get seller_id
     *
    ...
    {
        return $this->ci->session->userdata('seller_id');
    }

    public function is_admin()
    {
        return ($this->get_role_id() == 3) ? true : false;
    }
     ...
     * @return    array
     */
    // public function create_user($username, $first_name, $last_name, $email, $phone, $seller_id, $password, $email_activation)
    public function create_user($username, $first_name, $last_name, $email, $phone, $firebase_uid, $auth_provider, $password, $email_activation)
    {
        if ((strlen($username) > 0) and !$this->ci->users->is_username_available($username)) {
            $this->error = array('username' => 'auth_username_in_use');
        } elseif (!$this->ci->users->is_email_available($email)) {
            $this->error = array('email' => 'auth_email_in_use');
        } elseif ($phone != '' && !$this->ci->users->is_phone_available($phone)) {
            $this->error = array('phone' => 'auth_phone_in_use');
        } else {
            if ($password != '') {
