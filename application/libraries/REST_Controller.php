 ...
 * A fully RESTful server implementation for CodeIgniter using one library, one config file and multiple controller.
 *
 * @package         CodeIgniter
 ...
 */class REST_Controller extends CI_Controller
{
    ...
    public function __construct($config = 'rest')
    {
        ...
        // Get the login token name variable set in the rest config file
        $lgoin_token_variable = $this->config->item('lgoin_token_variable');
        // Work out the name of the SERVER entry based on config
        $lgoin_token_name = 'HTTP_' . strtoupper(str_replace('-', '_', $lgoin_token_variable));
        $this->rest->login_token = isset($this->_args[$lgoin_token_variable]) ? $this->_args[$lgoin_token_variable] : $this->input->server($lgoin_token_name);
        ...
        if (isset($this->_head_args[strtolower($this->config->item('lgoin_id_variable'))])) {
            $this->user_id = $this->_head_args[strtolower($this->config->item('lgoin_id_variable'))];
        }
        if (isset($this->_head_args[strtoupper($this->config->item('lgoin_id_variable'))])) {
            $this->user_id = $this->_head_args[strtoupper($this->config->item('lgoin_id_variable'))];
        }
        if (isset($this->_head_args[$this->config->item('lgoin_id_variable')])) {
            $this->user_id = $this->_head_args[$this->config->item('lgoin_id_variable')];
        }
     ...
     */
    public function convert_msg($data = null)
    {
        $response = "";
        if (is_array($data)) {
            $response = implode("\n", $data);
    ...
    protected function _detect_api_key()
    {
        ...
        $key = 'kkcoswggwgwkkc8w4ok808o48kswc40c0www4wss';
        if ($key) {
    ...
    protected function _Check_Auth_Token()
    {
//        return 16;
        // if (!$this->user_id) {
        //     $this->response(['status' => false, 'message' => "User ID not found"], REST_Controller::HTTP_UNAUTHORIZED);
        // }
        if (!isset($this->rest->login_token)) {
            $this->response(['status' => false, 'message' => "Login token not found"], REST_Controller::HTTP_UNAUTHORIZED);
        }
        $token_data = $this->Rest_model->get_token($this->rest->login_token);

        if (!$token_data) {
            $this->response(['status' => false, 'message' => "Invalid login token"], REST_Controller::HTTP_UNAUTHORIZED);
        }
        if ($token_data->user_id != $this->user_id) {
            $this->response(['status' => false, 'message' => "Invalid login tokens"], REST_Controller::HTTP_UNAUTHORIZED);
        }
        return $this->user_id;
    }
